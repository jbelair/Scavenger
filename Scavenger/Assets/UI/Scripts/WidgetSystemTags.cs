...
public class WidgetSystemTags : MonoBehaviour
{
    ...
    public void Set(List<DungeonType> types)
    {
        ...
        foreach (DungeonType type in types)
        {
            ...
            {
                Color colour = Schemes.Scheme(tag).colour;
                if (tag != "" 
                    && tag != type.category
                    && i < endIndex
                    && colour != Color.white 
                    && colour != Schemes.Scheme("rarity_abundant").colour 
                    && colour != Schemes.Scheme("rarity_common").colour 
                    && colour != Schemes.Scheme("rarity_uncommon").colour
                    && colour != Schemes.Scheme("rarity_rare").colour
                    && !tags.Contains(tag))
                {
