	...
	SubShader 
	{
		...
		int Type(fixed3 colour)
		{
			if (colour.r == colour.g && colour.r == colour.b)
			{
				if (colour.r > 0.76)
					return 0;
				else if (colour.r > 0.4)
					return 1;
				else if (colour.r > 0.24)
					return 2;
				else if (colour.r > 0)
					return 3;
				else
		...
		{
			if (type == 0)
				return tex2D(_Tex100, pos.xy * _Tex100_ST.xy + _Tex100_ST.zw);
			else if (type == 1)
				return tex2D(_Tex75, pos.xy * _Tex75_ST.xy + _Tex75_ST.zw);
			else if (type == 2)
				return tex2D(_Tex50, pos.xy * _Tex50_ST.xy + _Tex50_ST.zw);
			else if (type == 3)
				return tex2D(_Tex25, pos.xy * _Tex25_ST.xy + _Tex25_ST.zw);
			else if (type == 4)
				return tex2D(_Tex0, pos.xy * _Tex0_ST.xy + _Tex0_ST.zw);
			else
				return (1,1,1);
		}
