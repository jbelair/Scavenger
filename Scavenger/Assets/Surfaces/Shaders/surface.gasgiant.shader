...
#define F4 0.309016994374947451
	...
	void vert(inout appdata_full v, out Input o)
	{
		...
		o.texturePos = float3(v.texcoord.xy, 1 - pow(abs(v.vertex.y), 8));
		o.normal = v.normal;
	}
	...
	void surf(Input IN, inout SurfaceOutputStandard o)
	{
		...
		o.Albedo = temperature * lerp((albedo + tex1D(_Gasses, 1) * freznel + tex1D(_Gasses, 1) * IN.texturePos.z) / 2, tex1D(_Gasses, 1), 1 - IN.texturePos.z);
